								  ...
								  <div class="col-sm-10">
									  <select class="form-control" name="teacherid" id="teacherid">
										  <c:forEach items="${teacher}" var="item">
											 <option value="${item.userid}" ${item.userid==course.teacherid?"selected":"unselected"}>${item.username}</option>
										  </c:forEach>
									  </select>
								  </div>
							  </div>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<select class="form-control" name="coursetype" id="coursetype">
										...
										<option value="必修课" ${course.coursetype=="必修课"?"selected":"unselected"}>必修课</option>
										<option value="选修课" ${course.coursetype=="选修课"?"selected":"unselected"}>选修课</option>
										<option value="公共课" ${course.coursetype=="公共课"?"selected":"unselected"}>公共课</option>
									</select>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-10">
									<select class="form-control" name="collegeid">
										<c:forEach items="${college}" var="item">
											<option value="${item.collegeid}" ${item.collegeid==course.collegeid?"selected":"unselected"}>${item.collegename}</option>
										</c:forEach>
									</select>
								</div>
							</div>
